...
merge <- read.csv("Merged_synoptic.csv")

synoptic_clean <- filter(merge, !(Sample_Type == "RI") & !(Sample_Type == "CH"))
synoptic_clean <- filter(synoptic_clean, !(Site == "AG") & !(Site == "TR"))

synoptic_clean <- filter(synoptic_clean, CH4 > 0)

SW <- filter(synoptic_clean, Sample_Type == "SW")
...
# Should create a spreadsheet with all the atmospheric concentrations but for now....
