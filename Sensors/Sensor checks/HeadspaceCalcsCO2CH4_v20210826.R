...
# Last update: 20210826 by CLL

# Modified for Delmarva eosense CO2 sensor calibration checks
########################################
...
# same constants as in Demarty et al 2011 Biogeosciences, which referenes Lide 2007 Handbook of Chemistry and Physics
...
##### [7] FUNCTION TO CONVERT pCH4 from uatm to umol/m3 
...
# Carla working space
setwd("C:/Users/Carla López Lloreda/Dropbox/Grad school/Research/Delmarva project/Data") # **CHANGE WITH USERS**

# Read data for cal checks
data_cal <- read.csv("Sensors/Sensor checks/2021-05/JL cal 042021_GHG_GCHeadspace.csv")  # **CHANGE FOR SAMPLING MONTH**
...
air1 <- data_cal[ which(data_cal$Rep=="Air1"),]
air2 <- data_cal[ which(data_cal$Rep=="Air2"),]
room_air <- data_cal[ which(data_cal$Rep=="Room air"),]
# 
...
with(subset(air1, (Air_Location=="JL Air")), summary(CO2_ppm))
with(subset(air2, (Air_Location=="JL Air")), summary(CO2_ppm))
with(subset(room_air, (Air_Location=="Room")), summary(CO2_ppm))
...
write.csv(samp, "Sensors/Sensor checks/2021-05/2021-05_Sensor checks_GHG_Wetlands.csv")
