...
# Estimate metabolism for headwater wetlands
  ...
  # Added light model code from Hall & Hotchkiss 2017 via Yard et al.
	# Last updated: 20240514 by ERH from experimental pond code (cites above)
...
## These are all of the functions you'll need to "RUN" daily models to estimate GPP and ER in the ponds. Unless you want to update any of the functions, you can simply "select all" and "execute" this entire file. BUT: you should have a basic understanding of what each of these functions do so you can explain your methods and results!! -ERH, 2016-04-28
...
############################################
### [4] LOAD function to calculate BP
############################################

# FXN to caclulate BP with elevation (up to 2,000 m)
	# BP in mm Hg
	# elevation in m above sea level

BPcalc <- function (elev) {
	bp <- (-0.0818*elev) + 757.43
	bp
}

# fixed BP for experimental ponds in Umeå
# bp <- BPcalc(elev)

### / END BP function ###

############################################
### [5] LOAD functions for K600 & KO2
############################################

### [5a] K600 FUNCTION ###

# "kwind" == K600 calculated from wind speed 
# AS IN VACHON & PRAIRIE 2013 (instead of Cole & Caraco 1998)
# 2.51 (±0.99) + 1.48 (±0.34) · U10 + 0.39 (±0.08) · U10 · log10 LA
# NOW ONLY CALCULATE BASED ON 50% quantile of wind and pond area
# ponds are 8*10m = 8e-05 km2 ***NOTE THIS UPDATED LA - MH's CODE SAID 4e-05km. Better to FIX WITH TRUE VALUE.
# article k units are cm/hr; so recalc to m/hr by /100
# NOTE that lakeMET function below *24 to convert from m/hr to m/day
# ** updated code FIXES LA but multiplies K term in lakeMET by % surface that is ice free ***

#kwind <- function(wind, LA){
         #windm <- quantile(wind,c(0.5))
         #ka <- (2.51+ (1.48*windm) + (0.39*windm*log10(LA)))/100
         #ka
         #}
...
############################################
### [6] LOAD the main METABOLISM FUNCTION
############################################

### This is the pond/lake metabolism function, called "lakeMET"
...
lakeMET <- function(MET,oxy,zmix,temp,light,bp,ts,k600){
  ...
  # if using wind model to estimate k, which also requires k unit conversion *24 & wind as part of input call
  # for (i in 2:length(oxy)) { 
  #   metab[i]<-metab[i-1] + ((MET[1]/zmix)*(light[i]/sum(light))) + (MET[2]*(ts/zmix)) + ((Kcor(temp[i],kwind(wind))*24*ts*(osat(temp[i],bp)-metab[i-1]))/zmix) 
  # }
  
  # relies on a k600 input value for each day, units are m/h as for Kcor function (can be updated, but leave for now)
  for (i in 2:length(oxy)) { 
...
output <- data.frame(Wetland=(character(N)), Date=(character(N)), 
                     ...
                     elev=numeric(N), sig.50=numeric(N), 
